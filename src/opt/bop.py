...
def __moving_window_maximizer(top_slice, temporary_row, temp_row_campaign, budget_values, pedantic=False):
    ...
    :param pedantic: boolean parameter to print the steps of the algorithm, for tsg purposes only
...
def budget_optimizer(bb_matrices, budget_values, log=False, pedantic=False):
    ...
    :param log: boolean parameter indicating whether to print the result of the opt
    :param pedantic: boolean parameter to print the steps of the algorithm, for tsg purposes only
    ...
    if log or pedantic:
        ...
        print(
            ...
            *['\tcampaign {}:\t{}'.format(index, value) for index, value in zip(range(len(best_allocation)), best_allocation)],
            '\nvalue of the configuration: {0} [opt problem solved in {1:.3f} seconds]'.format(
